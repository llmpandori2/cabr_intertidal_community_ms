...
# make list of sites and spp to loop across
spp_list <- unique(target_90_home$Scientific_name) 
site_list <- unique(target_90_home$ZoneName)

# nested loop (i is for row # in table, j is for sites and k is for target taxa)
for (i in seq(1:15)){
  for(j in 1:length(site_list)) {
    ...
    filter(Scientific_name == spp_list[k] & 
           ZoneName == site_list[j])
    
    # run model
    gam_mod <- gam(Cover_mean ~ s(SurveyYear) + s(people), data = gam_data)
    
    # put relevant summary info in output table 
    gam_output$ZoneName[i] <- site_list[j]
    gam_output$Scientific_name[i] <- spp_list[k]
    ...
    p1 <- ggplot(data = gam_data,
           mapping = aes(x = SurveyYear, y = Cover_mean)) + 
            ...
            xlab('Year') + 
            ylab('Percent cover') + 
            ggtitle(paste('GAM Result for', spp_list[k], 'in', site_list[j]), 
...
remove(p1, p2, p3, site_list, spp_list, gam_data, gam_mod)

# make a pretty table using gam_output
