...
```{r tidy photoplot data}

# retain plot level data and original taxonomic specificity
target <- 
...
# see which plots were surveyed each years
  # notes: 
  # Po plots not established until 1995
  # No data for 2018?
  # where is M6 in Z1?
  # Exclude CAB2 Po7, Po8
plots <- ungroup(target_1990) %>%
  select(SiteCode, SurveyYear, Plot_code) %>%
  distinct() %>%
  mutate(surveyed = 'X') %>%
  pivot_wider(names_from = SurveyYear, values_from = surveyed)


# figure out if we need to add 0's to percent cover in 1990-2000 data
target_expand <- expand(ungroup(target_1990), ZoneName, SurveyYear, Plot_code, SpeciesCode)
