...
# original code follows this comment, replaced by above
...
I suggest having a different model for each plot type (like the linear models) b/c diffs between plot types swamp other effects. 
...
species.mat<-sqrt(sil.species)

# test env drivers next
fit<-envfit(speciesMDS, sil.env)
...
penvironsil<-ggplot(data=species.plot, aes(NMDS1, NMDS2))+
...
NMDS1<-musMDS$points[,1]
NMDS2<-musMDS$points[,2]
species.plot<-cbind(sil.id, NMDS1, NMDS2)
...
# run permanova 
species.dist<-vegdist(mus.species, method='bray')

species.div<-adonis2(species.dist ~ ZoneName, data = mus.id, permutations = 999, method='bray')
species.div
...
fit<-envfit(speciesMDS, mus.env)
arrow<-data.frame(fit$vectors$arrows,R = fit$vectors$r, P = fit$vectors$pvals)
arrow$FG <- rownames(arrow)
arrow.p<-filter(arrow, P <= 0.05)

penvironmus<-ggplot(data=species.plot, aes(NMDS1, NMDS2))+
  geom_point(data=species.plot, aes(NMDS1, NMDS2, color=ZoneName),position=position_jitter(.1))+##separates overlapping points
  stat_ellipse(aes(fill=ZoneName), alpha=.2,type='t',size =1, geom="polygon")+ ##changes shading on ellipses
  theme_minimal()+
  geom_segment(data=arrow.p, aes(x=0, y=0, xend=NMDS1, yend=NMDS2, label=FG, lty=FG), arrow=arrow(length=unit(.2, "cm")*arrow.p$R))+##add arrows (scaled by R-squared value)
...
# species matrix (think of this matrix like a single spp in the linear models)
barn.species <- target_shore %>%
  filter(Zone == 'CHT') %>%
  select(-c(SiteCode:Zone, people))

# environmental factors matrix (want to test for effects of zone, people, year - like single-spp linear models)
barn.env <- target_shore %>%
  filter(Zone == 'CHT') %>%
  select(SiteName, SurveyYear, people)

# identifier column matrix 
barn.id <- target_shore %>%
  filter(Zone == 'CHT') %>%
  select(SiteCode:Zone)

### Run NMDS (this is directly from your code - well written!) 
# I think your model may have had insufficient data b/c, when using the home plot data, there was only one spp per plot
musMDS<-metaMDS(barn.species, distance= "bray", k = 2, trymax = 35, autotransform = T)
...
# run permanova 
species.dist<-vegdist(barn.species, method='bray')

species.div<-adonis2(species.dist ~ ZoneName, data = barn.id, permutations = 999, method='bray')
species.div
...
#standardize with sqrt
species.mat<-sqrt(barn.species)

# test env drivers next
#try to fit environmental variables, can't get fit to work. "Missing value where TRUE/FALSE needed" error. Might be issue with NAs.
fit<-envfit(speciesMDS, barn.env)
arrow<-data.frame(fit$vectors$arrows,R = fit$vectors$r, P = fit$vectors$pvals)
arrow$FG <- rownames(arrow)
arrow.p<-filter(arrow, P <= 0.05)

penvironbarn<-ggplot(data=species.plot, aes(NMDS1, NMDS2))+
  geom_point(data=species.plot, aes(NMDS1, NMDS2, color=ZoneName),position=position_jitter(.1))+##separates overlapping points
  stat_ellipse(aes(fill=ZoneName), alpha=.2,type='t',size =1, geom="polygon")+ ##changes shading on ellipses
  theme_minimal()+
  geom_segment(data=arrow.p, aes(x=0, y=0, xend=NMDS1, yend=NMDS2, label=FG, lty=FG), arrow=arrow(length=unit(.2, "cm")*arrow.p$R))+##add arrows (scaled by R-squared value)
  labs(title="Barnacles")
penvironbarn
```

#tetrub- going to be the same as CHT
```{r}
### Create 3 matrices: species, environment and identifier columns

# species matrix (think of this matrix like a single spp in the linear models)
tet.species <- target_shore %>%
  filter(Zone == 'CHT') %>%
  select(-c(SiteCode:Zone, people))

# environmental factors matrix (want to test for effects of zone, people, year - like single-spp linear models)
tet.env <- target_shore %>%
  filter(Zone == 'CHT') %>%
  select(SiteName, SurveyYear, people)
...
### Run NMDS (this is directly from your code - well written!) 
# I think your model may have had insufficient data b/c, when using the home plot data, there was only one spp per plot
musMDS<-metaMDS(tet.species, distance= "bray", k = 2, trymax = 35, autotransform = T)
...
# run permanova 
species.dist<-vegdist(tet.species, method='bray')

species.div<-adonis2(species.dist ~ ZoneName, data = tet.id, permutations = 999, method='bray')
species.div
...
#standardize with sqrt
species.mat<-sqrt(tet.species)

# test env drivers next
#try to fit environmental variables, can't get fit to work. "Missing value where TRUE/FALSE needed" error. Might be issue with NAs.
fit<-envfit(speciesMDS, tet.env)
arrow<-data.frame(fit$vectors$arrows,R = fit$vectors$r, P = fit$vectors$pvals)
arrow$FG <- rownames(arrow)
arrow.p<-filter(arrow, P <= 0.05)

penvirontet<-ggplot(data=species.plot, aes(NMDS1, NMDS2))+
  geom_point(data=species.plot, aes(NMDS1, NMDS2, color=ZoneName),position=position_jitter(.1))+##separates overlapping points
  stat_ellipse(aes(fill=ZoneName), alpha=.2,type='t',size =1, geom="polygon")+ ##changes shading on ellipses
  theme_minimal()+
  geom_segment(data=arrow.p, aes(x=0, y=0, xend=NMDS1, yend=NMDS2, label=FG, lty=FG), arrow=arrow(length=unit(.2, "cm")*arrow.p$R))+##add arrows (scaled by R-squared value)
...
# species matrix (think of this matrix like a single spp in the linear models)
pol.species <- target_shore %>%
  filter(Zone == 'POL') %>%
  select(-c(SiteCode:Zone, people))

# environmental factors matrix (want to test for effects of zone, people, year - like single-spp linear models)
pol.env <- target_shore %>%
  filter(Zone == 'POL') %>%
  select(SiteName, SurveyYear, people)
...
### Run NMDS (this is directly from your code - well written!) 
# I think your model may have had insufficient data b/c, when using the home plot data, there was only one spp per plot
musMDS<-metaMDS(pol.species, distance= "bray", k = 2, trymax = 35, autotransform = T)
...
# run permanova 
species.dist<-vegdist(pol.species, method='bray')

species.div<-adonis2(species.dist ~ ZoneName, data = pol.id, permutations = 999, method='bray')
species.div
...
#standardize with sqrt
species.mat<-sqrt(pol.species)

# test env drivers next
#try to fit environmental variables, can't get fit to work. "Missing value where TRUE/FALSE needed" error. Might be issue with NAs.
fit<-envfit(speciesMDS, pol.env)
arrow<-data.frame(fit$vectors$arrows,R = fit$vectors$r, P = fit$vectors$pvals)
arrow$FG <- rownames(arrow)
arrow.p<-filter(arrow, P <= 0.05)

penvironpol<-ggplot(data=species.plot, aes(NMDS1, NMDS2))+
  geom_point(data=species.plot, aes(NMDS1, NMDS2, color=ZoneName),position=position_jitter(.1))+##separates overlapping points
  stat_ellipse(aes(fill=ZoneName), alpha=.2,type='t',size =1, geom="polygon")+ ##changes shading on ellipses
  theme_minimal()+
  geom_segment(data=arrow.p, aes(x=0, y=0, xend=NMDS1, yend=NMDS2, label=FG, lty=FG), arrow=arrow(length=unit(.2, "cm")*arrow.p$R))+##add arrows (scaled by R-squared value)
  labs(title="Pollicipes")
penvironpol
