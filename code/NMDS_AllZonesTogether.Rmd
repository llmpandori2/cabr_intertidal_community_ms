...
targetlist <- c('CHTBAL', 'MUSSEL', 'SILCOM', 'POLPOL')
...
gam_output <- tibble(Scientific_name = target_90_home$Scientific_name) %>%
              distinct() %>%
...
for (i in 1:nrow(gam_output)) {
    ...
    ggsave(filename = paste('figures/GAM/gam_cover_people_time_2', 
                 gsub('/', '', gam_data$Scientific_name[i]),  '.png', sep = ''),
           plot = p3,
           width = 12)
}
...
# make for all 
gam_time_fig(target_90_home, 'time')

# make for 4 types of spp for graph
gam_time_fig(filter(target_90_home, SpeciesCode != 'TETRUB'), 'no_tet')
