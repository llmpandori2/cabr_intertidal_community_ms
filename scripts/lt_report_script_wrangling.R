...
# no fall data for 2018 or 2009, instead use spring data for following year (sp10 and sp19) - closer in time (~ 4 mo) than same year spring data (not available for 2018)
...
target <- target %>%
  ...
  filter(site_code %in% cabrsites & survey_year < 2021 
        ...
        & (if_else(survey_year == 2019 | survey_year == 2010, 
                   season_name %in% c('Spring', 'Fall'), season_name == 'Fall')) 
  ...
  mutate(scientific_name = str_to_sentence(scientific_name),
         # change survey year for sp 2019 and sp 2010 to prior yr
         survey_year = if_else(season_name == 'Spring', 
                               (survey_year -1), survey_year)) %>%
